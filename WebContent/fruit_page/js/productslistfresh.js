	...
	if(!document.getElementById('veg-is-empty')){
		$(window).scroll(function(event) {
			var s_t=document.documentElement.scrollTop||document.body.scrollTop,
				...
				if(s_t+c_h>list_bottom&&$loading.css('display')!='block'&&$('body').data('freshlast')!=true&&$('body').data('freshlast')!="true"){
					$loading.fadeIn(1000);
					/*alert($('body').data('freshlast'))*/
					setTimeout(function(){
						$.ajax({
							url: 'findFreshVegetablesFlow',
							type: 'GET',
							dataType: 'json',
							data: {},
							success:function(req){
								var json=req.data,
									flag=0,
									len=req.data.length;
								if(/^true$/.test(req.freshisLast)){
									$('body').data('freshlast','true');
								}
