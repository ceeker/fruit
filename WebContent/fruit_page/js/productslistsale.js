	...
	if(!document.getElementById('veg-is-empty')){
		$(window).scroll(function(event) {
			var s_t=document.documentElement.scrollTop||document.body.scrollTop,
				...
				if(s_t+c_h>list_bottom&&$loading.css('display')!='block'&&$('body').data('hotlast')!=true&&$('body').data('hotlast')!="true"){
					//alert($('body').data('hotlast'))
					$loading.fadeIn(1000);
					setTimeout(function(){
						$.ajax({
							url: 'findSaleVegetablesFlow',
							type: 'GET',
							dataType: 'json',
							data: {},
							success:function(req){
								var json=req.data,
									flag=0,
									len=req.data.length;
								if(/^true$/.test(req.hotisLast)){
									$('body').data('hotlast','true');
								}
